...
router.patch("/user/:userId/limits", authenticateAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
        const { companyLimit, contactLimit, reviewLimit, plan, duration } = req.body;
        ...
        const allowedDurations = ["monthly", "annually"];
        ...
        await logAdminAction({
            adminId:     req.admin.id,
            action:      ADMIN_ACTIONS.EDIT_USER_LIMITS || "EDIT_USER_LIMITS",
            targetType:  "user",
            targetId:    userId,
            targetName:  user.name || user.email,
        ...
        res.json({
            ...
            planRecord: {
                id:        newUserPlan.id,
                plan:      newUserPlan.planType,
                duration:  newUserPlan.duration,
                startDate: newUserPlan.startDate,
                endDate:   newUserPlan.endDate,
                status:    newUserPlan.status,
            },
        });
    } catch (err) {
...
function buildNotes(actionType, previousPlan, currentPlan, duration) {
    const map = {
        renew:     `Plan renewed: ${currentPlan} (${duration})`,
        upgrade:   `Upgraded from ${previousPlan} → ${currentPlan} (${duration})`,
        downgrade: `Downgraded from ${previousPlan} → ${currentPlan}${currentPlan === "free" ? " (cancelled paid plan)" : ` (${duration})`}`,
        adjust:    `Limits adjusted on existing ${currentPlan} plan`,
    };
    return map[actionType] || "";
}

function buildSuccessMessage(actionType, plan, duration, endDate) {
    const expiry = endDate
        ? ` — expires ${new Date(endDate).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}`
        : "";

    const map = {
        renew:     `Plan renewed: ${plan} (${duration})${expiry}`,
        upgrade:   `Plan upgraded to ${plan} (${duration})${expiry}`,
        downgrade: plan === "free"
            ? "Plan cancelled and reset to Free"
            : `Plan downgraded to ${plan} (${duration})${expiry}`,
        adjust:    "Limits updated successfully",
    };
    return map[actionType] || "Plan updated successfully";
}
...
router.post("/user/:userId/plan-renew", authenticateAdmin, async (req, res) => {
    try {
        ...
        res.json({
            success:    true,
            message:    `Plan renewed successfully — expires ${endDate.toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}`,
            actionType: "renew",
            planRecord: {
                id:        renewRecord.id,
                ...
                status:    renewRecord.status,
            },
        });
    } catch (err) {
        console.error("Error renewing plan:", err);
        res.status(500).json({ message: "Error renewing plan" });
    }
});
...
router.get("/user/:userId/plan-history", authenticateAdmin, async (req, res) => {
    try {
        ...
        const history = await UserPlan.findAll({
            where: { userId },
            order: [["createdAt", "DESC"]],
        });

        // Check if active plan has expired and auto-update if so
        const activePlan = history.find(h => h.status === "active");
        if (activePlan?.endDate && new Date() > new Date(activePlan.endDate)) {
            await activePlan.update({ status: "expired" });
            activePlan.status = "expired"; // reflect in response
        }
        ...
        res.status(500).json({ message: "Error fetching plan history" });
    }
});
