  ...
  <div v-if="activeTab === 'contact'" :class="['contact-tab', { 'dark-mode': isDarkMode }]">
    ...
    <div v-if="!showContactForm" class="list-view">
      ...
      <div class="list-header">
        <div class="list-header-left">
          ...
          <span class="count-pill">{{ contacts.length }} <span style="opacity:0.5">/</span> {{
              userLimits.contactLimit
            }} contacts</span>
        </div>
        <button
      ...
      <!-- Search -->
      <div v-if="contacts.length > 0" class="search-bar">
        <svg class="search-icon" fill="none" height="16" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
             width="16">
        ...
        <input
            ...
            type="text"
        />
        <button v-if="contactSearch" class="search-clear" type="button" @click="contactSearch = ''">
          <svg fill="none" height="14" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="14">
        ...
        >
          ...
          <div class="card-top">
            ...
            <div class="card-identity">
              ...
              <span v-if="c.Company?.companyName" class="company-tag">
            ...
            <div class="contact-meta">
              <div class="meta-row">
                <svg fill="none" height="13" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="13">
                  <path
                      d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 11.9a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
                </svg>
                <a :href="`/${c.mobile?.replace(/\D/g,'')}`" class="meta-link" target="_blank">{{ c.mobile }}</a>
              </div>
              <div class="meta-row">
                <svg fill="none" height="13" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="13">
            ...
            <div class="wallet-row">
              <div class="wallet-group">
                <button class="wallet-btn google"
                        ...
                        @click="isFree ? showAlert({ type: 'warning', title: 'Upgrade Required', message: 'Google Wallet is available on paid plans only. Please upgrade to use this feature.', confirmLabel: 'OK' }) : saveToGoogleWallet(c)">
                  <svg fill="none" height="13" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="13">
                ...
                <button class="wallet-btn google-copy"
                        title="Copy Google Wallet link"
                        @click="isFree ? showAlert({ type: 'warning', title: 'Upgrade Required', message: 'Google Wallet links are available on paid plans only. Please upgrade to use this feature.', confirmLabel: 'OK' }) : copyGoogleWalletLink(c)">
                  <svg fill="none" height="12" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="12">
              ...
              <div class="wallet-group">
                <button :class="{ disabled: !appleConfigured }" :disabled="!appleConfigured" :title="appleConfigured ? 'Download Apple Wallet pass' : 'Apple Wallet not configured'"
                        class="wallet-btn apple"
                        @click="isFree ? showAlert({ type: 'warning', title: 'Upgrade Required', message: 'Apple Wallet is available on paid plans only. Please upgrade to use this feature.', confirmLabel: 'OK' }) : saveToAppleWallet(c)">
                  <svg fill="none" height="13" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="13">
                ...
                <button :class="{ disabled: !appleConfigured }" :disabled="!appleConfigured"
                        :title="appleConfigured ? 'Download .pkpass file' : 'Apple Wallet not configured'"
                        class="wallet-btn apple-copy"
                        @click="isFree ? showAlert({ type: 'warning', title: 'Upgrade Required', message: 'Apple Wallet downloads are available on paid plans only. Please upgrade to use this feature.', confirmLabel: 'OK' }) : copyAppleWalletLink(c)">
                  <svg fill="none" height="12" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="12">
          ...
          <div class="card-actions">
            <a :href="`/${c.mobile?.replace(/\D/g,'')}`" class="action-btn view-btn" rel="noopener" target="_blank">
              <svg fill="none" height="13" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="13">
      ...
      <div v-else-if="contactSearch" class="empty-state">
        <div class="empty-icon-wrap">
          <svg fill="none" height="32" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" width="32">
            <circle cx="11" cy="11" r="8"/>
            <line x1="21" x2="16.65" y1="21" y2="16.65"/>
          </svg>
        </div>
        ...
        <button class="empty-action-btn" @click="contactSearch = ''">Clear search</button>
      </div>
      ...
      <div v-else class="empty-state">
        ...
        <button v-if="contactCount < userLimits.contactLimit" class="empty-action-btn" @click="showContactForm = true">
          + Add Contact
        </button>
      </div>
      ...
      <div v-if="contactTotalPages > 1" class="pagination-bar">
        <button :disabled="contactPage === 1" class="pag-btn arrow" @click="contactPage--">
        ...
        <button :disabled="contactPage === contactTotalPages" class="pag-btn arrow" @click="contactPage++">
          <svg fill="none" height="16" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="16">
    ...
    <div v-else class="form-view">
      ...
      <div class="form-topbar">
        <button class="back-btn" type="button" @click="showContactForm = false; resetForm()">
        ...
        <button :disabled="saving" class="save-btn-top" type="button" @click="saveContact">
          <svg v-if="!saving" fill="none" height="15" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
               width="15">
      ...
      <div class="form-body">
        ...
        <div class="form-section">
          ...
          <div class="photo-upload-area">
            <div v-if="photoPreview" class="photo-preview-wrap">
              ...
              <button class="photo-remove-btn" type="button" @click="removePhoto">
            ...
            <label class="photo-dropzone" for="photo-upload">
              <input id="photo-upload" accept="image/*" class="hidden-file" type="file" @change="handlePhotoUpload"/>
              <div class="dropzone-inner">
                <svg fill="none" height="26" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" width="26">
                  <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
          ...
          <div class="form-section">
            ...
            <div class="fields-grid">
              <div class="field-wrap">
                ...
                <input v-model="contactForm.firstName" class="field-input" placeholder="John" required type="text"/>
              </div>
              <div class="field-wrap">
                <label class="field-label">Last Name <span class="req">*</span></label>
                <input v-model="contactForm.lastName" class="field-input" placeholder="Doe" required type="text"/>
              </div>
              <div class="field-wrap">
                <label class="field-label">Email <span class="req">*</span></label>
                <input v-model="contactForm.email" class="field-input" placeholder="john@example.com" required
                       type="email"/>
              </div>
              <div class="field-wrap">
                <label class="field-label">Designation <span class="req">*</span></label>
                <input v-model="contactForm.designation" class="field-input" placeholder="e.g. Sales Manager"
                       required type="text"/>
              </div>
              <div class="field-wrap">
              ...
              <div class="field-wrap">
                ...
                <div class="status-toggle-group">
                  <button :class="['status-toggle-btn', { active: contactForm.status === 'active' }]" type="button"
                          @click="contactForm.status = 'active'">
                    <span class="toggle-dot active-dot"></span> Active
                  </button>
                  <button :class="['status-toggle-btn', { active: contactForm.status === 'inactive' }]" type="button"
                          @click="contactForm.status = 'inactive'">
            ...
            <div class="fields-grid">
              ...
              <div class="field-wrap full">
                ...
                <div class="phone-row">
                  ...
                  <input v-model="contactForm.mobile" class="field-input phone-num-input" placeholder="Enter mobile number"
                         required type="tel" @input="handleContactMobile"/>
                </div>
                <p v-if="phoneValidation.mobile.message && !checkingMobile" :class="phoneValidation.mobile.isValid ? 'success' : 'error'"
                   class="field-note">
                  {{ phoneValidation.mobile.message }}
                </p>
                <p v-if="mobileExistsMessage" :class="mobileExists ? 'error' : checkingMobile ? 'info' : 'success'"
                   class="field-note">
                  {{ mobileExistsMessage }}
                </p>
              </div>
              ...
              <div class="field-wrap full">
                ...
                <div class="url-preview-row">
                  <input :value="publicCardUrl" class="field-input url-readonly" readonly type="text"
                         @click="$event.target.select()"/>
                  <button class="url-icon-btn copy" title="Copy URL" type="button" @click="copyPublicCardUrl">
                    <svg fill="none" height="15" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="15">
                      <rect height="13" rx="2" width="13" x="9" y="9"/>
                      <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
                    </svg>
                  </button>
                  <a v-if="contactForm.mobile && phoneValidation.mobile.isValid" :href="publicCardUrl" class="url-icon-btn open"
                     target="_blank" title="Open card">
                    <svg fill="none" height="15" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="15">
              ...
              <div class="field-wrap">
                ...
                <div class="phone-row">
                  ...
                  <input v-model="contactForm.telephone" class="field-input phone-num-input" placeholder="Landline / alternate"
                         type="tel" @input="handleContactTelephone"/>
                </div>
                <p v-if="phoneValidation.telephone.message" :class="phoneValidation.telephone.isValid ? 'success' : 'error'"
                   class="field-note">
              ...
              <div class="field-wrap">
                <label class="field-label">
                  ...
                  <label class="same-toggle">
                    <input v-model="whatsappSameAsMobile" class="hidden-check" type="checkbox"/>
                    <span :class="{ 'toggle-on': whatsappSameAsMobile }" class="same-toggle-track">
                      <span :class="{ 'thumb-on': whatsappSameAsMobile }" class="same-toggle-thumb"></span>
                    </span>
                    Same as mobile
                  </label>
                </label>
                ...
                <p v-if="phoneValidation.whatsapp.message && !whatsappSameAsMobile" :class="phoneValidation.whatsapp.isValid ? 'success' : 'error'"
                   class="field-note">
              ...
              <div class="field-wrap full">
                ...
                <div class="input-icon-wrap">
                  <svg class="input-icon" fill="none" height="15" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                       width="15">
                    <path d="M21 11.5a8.5 8.5 0 1 1-3-6.5"/>
                    <path d="M16 8c-1.5-1.5-4-1-5 1s1 4 2 5 3 3 5 2 2.5-3.5 1-5"/>
                  </svg>
                  <input v-model="contactForm.whatsappChannel" class="field-input icon-padded" placeholder="https://whatsapp.com/channel/..."
                         type="url"/>
                </div>
        ...
        <template v-else-if="contactType === 'group'">
          ...
          <div class="form-section">
            ...
            <div class="fields-grid">
              ...
              <div class="field-wrap">
                <label class="field-label">Email <span class="req">*</span></label>
                <input v-model="contactForm.email" class="field-input" placeholder="group@example.com" required
                       type="email"/>
              </div>
              <div class="field-wrap">
                <label class="field-label">Designation <span class="req">*</span></label>
                <input v-model="contactForm.designation" class="field-input" placeholder="e.g. Team Lead" required
                       type="text"/>
              </div>
              <div class="field-wrap">
                <label class="field-label">Company</label>
        ...
        <div class="form-bottom-actions">
          <button :disabled="saving" class="cancel-btn" type="button" @click="showContactForm = false; resetForm()">
            Cancel
          </button>
          <button :disabled="saving" class="save-btn" type="button" @click="saveContact">
            <svg v-if="!saving" fill="none" height="16" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                 width="16">
              <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/>
              <polyline points="17 21 17 13 7 13 7 21"/>
            </svg>
            <svg v-else class="spin-icon" fill="none" height="16" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                 width="16">
              <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
            </svg>
      ...
      <div v-if="showQrPopup" class="qr-overlay" @click.self="showQrPopup = false">
        <div :class="['qr-modal', { 'dark-mode': isDarkMode }]">
          <div class="qr-modal-header">
            ...
            <button class="qr-close-btn" type="button" @click="showQrPopup = false">
              <svg fill="none" height="18" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="18">
                <line x1="18" x2="6" y1="6" y2="18"/>
                <line x1="6" x2="18" y1="6" y2="18"/>
              </svg>
          ...
          <div class="qr-modal-actions">
            <button class="qr-action-btn download" type="button" @click="downloadQr">
              <svg fill="none" height="15" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="15">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                <polyline points="7 10 12 15 17 10"/>
                <line x1="12" x2="12" y1="15" y2="3"/>
            ...
            <button class="qr-action-btn share" type="button" @click="sharePoster">
              <svg fill="none" height="15" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" width="15">
...
import { useWalletAccess } from '@/composables/useWalletAccess.js';
...
const props = defineProps({
  ...
  userPlan: {type: String, default: 'free'}   // ← ADD THIS
});

const isFree = computed(() => (props.userPlan || 'free').toLowerCase() === 'free');
...
async function saveToGoogleWallet(contact) {
  // ✅ Block if not on paid plan
  if (!canUseGoogleWallet.value) {
    await showAlert({
      type: 'warning',
      title: 'Upgrade Required',
      message: 'Google Wallet is available on paid plans only. Please upgrade to use this feature.',
  ...
  try {
    ...
    if (res.data.saveUrl) {
      ...
      try {
        await api.post(
            `/wallet/google/track/${walletContact.phone.replace(/\D/g, '')}`,
            { userAgent: navigator.userAgent }
        );
      } catch (trackErr) {
        // Tracking failure shouldn't block user
        console.warn('Analytics tracking failed:', trackErr.message);
      }

      // Open wallet on mobile, new window on desktop
      ...
      console.log('✅ Google Wallet opened successfully');

    } else {
      await showAlert({
        type: 'error',
        title: 'Wallet Error',
        message: 'Could not generate Google Wallet link.',
        confirmLabel: 'OK'
      });
    }
    ...
    if (err.response?.status === 403 && err.response?.data?.requiresUpgrade) {
      ...
      await showAlert({
        type: 'error',
        title: 'Wallet Error',
        message: err.response?.data?.message || 'Failed to generate Google Wallet link.',
        confirmLabel: 'OK'
      });
    }
...
async function copyGoogleWalletLink(contact) {
  ...
  if (!canUseGoogleWallet.value) {
    await showAlert({
      type: 'warning',
      title: 'Upgrade Required',
      message: 'Google Wallet links are available on paid plans only. Please upgrade to use this feature.',
      confirmLabel: 'Upgrade'
    });
    return;
  }
  ...
  try {
    ...
    if (res.data.saveUrl) {
      ...
      await showAlert({
        type: 'success',
        title: 'Link Copied!',
        message: 'Google Wallet link copied! Send it to anyone — they tap to add to their wallet.',
        confirmLabel: 'Great!'
      });

      console.log('✅ Google Wallet link copied');
    ...
    if (err.response?.status === 403 && err.response?.data?.requiresUpgrade) {
      ...
      await showAlert({
        type: 'error',
        title: 'Wallet Error',
        message: err.response?.data?.message || 'Failed to copy Google Wallet link.',
        confirmLabel: 'OK'
      });
    }
    console.error('❌ Copy Google Wallet link error:', err);
  }
}

async function saveToAppleWallet(contact) {
  // ✅ Block if not on paid plan
  if (!canUseAppleWallet.value) {
    await showAlert({
      type: 'warning',
      title: 'Upgrade Required',
      message: 'Apple Wallet is available on paid plans only. Please upgrade to use this feature.',
      confirmLabel: 'Upgrade'
    });
    return;
  }
  ...
  if (!appleConfigured.value) {
    await showAlert({
      ...
      message: 'Apple Wallet is not configured yet. Please contact support.',
      confirmLabel: 'OK'
    });
    return;
  }
  ...
  try {
    const response = await api.post(
        '/wallet/apple/pass',
        { contact: buildWalletContact(contact) },
        { responseType: 'blob' }
    );
    ...
    link.href = url;
    link.download = `${(contact.firstName || 'card')}_${(contact.lastName || '')}.pkpass`
        .replace(/\s+/g, '_');
    ...
    console.log('✅ Apple Wallet pass downloaded');
    ...
    if (err.response?.status === 403 && err.response?.data?.requiresUpgrade) {
      ...
      // Parse error message from blob if needed
...
async function copyAppleWalletLink(contact) {
  ...
  if (!canUseAppleWallet.value) {
    await showAlert({
      type: 'warning',
      title: 'Upgrade Required',
      message: 'Apple Wallet downloads are available on paid plans only. Please upgrade to use this feature.',
      confirmLabel: 'Upgrade'
    });
    return;
  }
  ...
  try {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `${contact.firstName || 'card'}_${contact.lastName || ''}_wallet.pkpass`
        .replace(/\s+/g, '_');
    ...
    console.log('✅ Apple Wallet file downloaded');
    ...
    console.error('❌ Copy Apple Wallet link error:', err);
  }
}
...
async function copyPublicCardUrl() {
  if (!contactForm.value.mobile) {
    await showAlert({
      type: 'warning',
      title: 'No Number',
      message: 'Please enter a mobile number first.',
      confirmLabel: 'OK'
    }); // ← NEW
    return;
  }
  try {
    ...
    await showAlert({
      type: 'success',
      title: 'Copied!',
      message: 'Public card URL copied to clipboard.',
      confirmLabel: 'OK'
    }); // ← NEW
  } catch {
    await showAlert({
      type: 'error',
      title: 'Copy Failed',
      message: 'Failed to copy URL. Please try manually.',
      confirmLabel: 'OK'
    }); // ← NEW
  }
}
...
async function saveContact() {
  ...
  if (!mobileVal.isValid) {
    await showAlert({
      ...
      message: `Mobile number error: ${mobileVal.error}`,
      confirmLabel: 'OK'
    });
    return;
  }
  ...
  if (contactForm.value.telephone) {
    ...
    if (!telV.isValid) {
      await showAlert({
        ...
        message: `Telephone error: ${telV.error}`,
        confirmLabel: 'OK'
      });
      return;
    }
  ...
  if (!whatsappSameAsMobile.value && contactForm.value.whatsapp) {
    ...
    if (!wV.isValid) {
      await showAlert({
        ...
        message: `WhatsApp error: ${wV.error}`,
        confirmLabel: 'OK'
      });
      return;
    }
  ...
  if (contactForm.value.whatsappChannel?.trim() && !/^https?:\/\/.+/i.test(contactForm.value.whatsappChannel.trim())) {
    await showAlert({
      type: 'warning',
      title: 'Invalid URL',
      message: 'WhatsApp Channel must be a valid URL starting with http:// or https://',
      confirmLabel: 'OK'
    });
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      await showAlert({
        type: 'error',
        title: 'Save Failed',
        message: data.message || 'Failed to save contact. Please try again.',
        confirmLabel: 'OK'
      }); // ← NEW
    }
  } catch (err) {
...
.wallet-btn.google-copy:hover {
  background: var(--c-google-copy);
  color: #fff;
}
...
.wallet-btn.apple:hover:not(.disabled) {
...
.wallet-btn.apple-copy:hover:not(.disabled) {
  background: var(--c-text-primary);
  color: var(--c-surface);
}
...
.empty-action-btn:hover {
  background: var(--c-accent-hover);
  box-shadow: 0 4px 14px rgba(124, 92, 78, 0.35);
}
...
.photo-remove-btn:hover {
  ...
  box-shadow: 0 4px 10px rgba(184, 50, 50, 0.4);
}
...
.dark-mode .url-preview-row {
  border-color: rgba(96, 168, 232, 0.2);
}
