          ...
          <aside :class="['settings-sidebar', { 'mobile-hidden': mobileShowContent }]">
            <nav class="settings-nav">
              ...
              >
                ...
                <svg class="nav-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                     stroke-width="2">
            ...
            <div v-if="activeSetting === 'username'" class="settings-panel">
              ...
              <div class="panel-body">
                <div class="form-group">
                  ...
                  <input v-model="settingsForm.username" class="form-input" placeholder="Enter new username"
                         type="text"/>
                </div>
                <button :disabled="usernameLoading" class="save-btn full-width" type="button" @click="updateUsername">
                  <svg v-if="!usernameLoading" fill="none" height="15" stroke="currentColor" stroke-width="2"
                       viewBox="0 0 24 24" width="15">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                  <svg v-else class="spin-icon" fill="none" height="15" stroke="currentColor" stroke-width="2"
                       viewBox="0 0 24 24" width="15">
                    <path d="M21 12a9 9 0 1 1-6.219-8.56"/>
                  </svg>
                  {{ usernameLoading ? 'Saving…' : 'Save Username' }}
                </button>
                <p v-if="usernameMessage" :class="['feedback-msg', usernameSuccess ? 'msg-success' : 'msg-error']">
                  {{ usernameMessage }}</p>
              </div>
            </div>
            ...
            <div v-if="activeSetting === 'email'" class="settings-panel">
              ...
              <div class="panel-body">
                <div class="form-group">
                  ...
                  <input v-model="settingsForm.email" :disabled="emailOtpSent || emailVerified" class="form-input"
                         placeholder="Enter new email" type="email"/>
                </div>
                <button v-if="!emailOtpSent" :disabled="emailLoading" class="save-btn full-width" type="button"
                        @click="sendEmailOtp">
                ...
                <div v-if="emailVerified">
                  <button :disabled="emailLoading" class="save-btn full-width" type="button"
                          @click="confirmEmailChange">
                    {{ emailLoading ? 'Saving…' : 'Confirm Email Change' }}
                  </button>
                </div>
                <p v-if="emailMessage" :class="['feedback-msg', emailVerified ? 'msg-success' : 'msg-error']">
                  {{ emailMessage }}</p>
              </div>
            </div>
              ...
              <div class="panel-body">
                <div v-for="(field, key) in passwordFields" :key="key" class="form-group">
                  ...
                  <div class="password-wrap">
                    <input v-model="passwordForm[key]" :placeholder="field.placeholder"
                           :type="field.show ? 'text' : 'password'" class="form-input"/>
                    <button class="eye-btn" type="button" @click="field.show = !field.show">
                      <svg v-if="!field.show" fill="none" height="16" stroke="currentColor" stroke-width="2"
                ...
                <button :disabled="passwordLoading" class="save-btn full-width" type="button" @click="updatePassword">
                  <svg v-if="!passwordLoading" fill="none" height="15" stroke="currentColor" stroke-width="2"
                       viewBox="0 0 24 24" width="15">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                  <svg v-else class="spin-icon" fill="none" height="15" stroke="currentColor" stroke-width="2"
                ...
                <p v-if="passwordMessage" :class="['feedback-msg', passwordSuccess ? 'msg-success' : 'msg-error']">
                  {{ passwordMessage }}</p>
              </div>
            </div>
                ...
                <div class="panel-body">
                  <div class="limits-overview">
                    <div v-for="lim in limitCards" :key="lim.key" class="limit-card">
                      ...
                      <div class="limit-bar-track">
                        <div :class="['limit-bar-fill', lim.colorClass]"
                             :style="{ width: Math.min((lim.count / lim.max) * 100, 100) + '%' }"/>
                      </div>
                    </div>
                  </div>
                  <form class="request-form" @submit.prevent="submitSettingsRequest">
                    <div v-for="f in requestFields" :key="f.key" class="form-group">
                      ...
                      <div class="quantity-selector">
                        <button class="qty-btn" type="button" @click="decrement(f.key)">
                          <svg fill="none" height="13" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                               width="13">
                            <line x1="5" x2="19" y1="12" y2="12"/>
                          </svg>
                        </button>
                        <input v-model.number="settingsRequestForm[f.key]" class="qty-input" max="500" min="0"
                               type="number"/>
                    ...
                    <div class="form-group">
                      ...
                      <textarea v-model="settingsRequestForm.reason" class="form-input"
                                placeholder="Tell us why you need more limits…" rows="3"/>
                    </div>
                    <button
                        :disabled="settingsRequestLoading || (!settingsRequestForm.companies && !settingsRequestForm.contacts && !settingsRequestForm.reviews)"
                        class="save-btn full-width" type="submit">
                      <svg v-if="!settingsRequestLoading" fill="none" height="15" stroke="currentColor" stroke-width="2"
                           viewBox="0 0 24 24" width="15">
                    ...
                    <p v-if="settingsRequestMessage"
                       :class="['feedback-msg', settingsRequestSuccess ? 'msg-success' : 'msg-error']">
                      {{ settingsRequestMessage }}</p>
                  </form>
                </div>
                <div v-if="isFreeOrDemo" class="plan-gate-overlay">
                  <div class="plan-gate-inner">
                    ...
                    <h4 class="plan-gate-title">
                      {{
                        normalizedPlan === 'demo'
                            ? 'Demo Plan Limitations'
                            : 'Upgrade to Request Limits'
                      }}
                    </h4>
                    ...
                    <p class="plan-gate-desc">
                      {{
                        normalizedPlan === 'demo'
                            ? 'You are on the Demo Plan. Demo plan has limited features. Upgrade to a plan to unlock more companies, contacts, and reviews.'
                            : 'Free plan users cannot request additional limits. Choose a plan to unlock more companies, contacts, and reviews.'
                      }}
                    </p>
                    <button class="plan-gate-btn" type="button" @click="emit('open-upgrade'); emit('close')">
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                           stroke-width="2">
                        <path d="M12 2l2.9 6 6.6.9-4.8 4.7 1.1 6.6L12 17l-5.8 3.2 1.1-6.6L2.5 8.9l6.6-.9z"/>
                      </svg>
                      {{ normalizedPlan === 'demo' ? 'Upgrade Plan' : 'Choose a Plan' }}
                    </button>
                  </div>
                </div>
              </div>
                ...
                <div class="panel-body">
                  <div v-if="historyLoading" class="empty-state">
                    <div class="empty-icon-wrap spin-wrap">
                      <svg class="spin-icon" fill="none" height="26" stroke="currentColor" stroke-width="2"
                  ...
                  <div v-else-if="historyError" class="empty-state">
                    <div class="empty-icon-wrap" style="background: var(--c-danger-light); color: var(--c-danger);">
                      <svg fill="none" height="26" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                           width="26">
                        <circle cx="12" cy="12" r="10"/>
                  ...
                  <div v-else-if="!requestHistory.length" class="empty-state">
                    ...
                    <button class="save-btn" type="button" @click="activeSetting = 'limits'">Make Your First Request
                    </button>
                  </div>
                      ...
                      <div v-for="req in displayedHistory" :key="req.id" class="history-item">
                        ...
                        <div class="history-item-body">
                          <div class="detail-grid">
                            <div v-if="req.requestedCompanies > 0" class="detail-row"><span
                                class="detail-label">Companies</span><span class="detail-value">{{
                                req.requestedCompanies
                              }}</span></div>
                            <div v-if="req.requestedContacts > 0" class="detail-row"><span
                                class="detail-label">Contacts</span><span class="detail-value">{{
                                req.requestedContacts
                              }}</span></div>
                            <div v-if="req.requestedReviews > 0" class="detail-row"><span
                                class="detail-label">Reviews</span><span class="detail-value">{{
                                req.requestedReviews
                              }}</span></div>
                          </div>
                          <div v-if="req.reason" class="req-reason"><p>{{ req.reason }}</p></div>
                        </div>
                        <div class="history-item-footer"><span class="req-date">{{ formatDate(req.createdAt) }}</span>
                        </div>
                      </div>
                    </div>
                    <div v-if="!displayedHistory.length && historyFilter !== 'all'" class="no-results">No {{
                        historyFilter
                      }} requests found.
                    </div>
                  </div>
...
const normalizedPlan = computed(() =>
    (props.userPlan || 'free').toLowerCase()
)

const isFreeOrDemo = computed(() =>
    normalizedPlan.value === 'free' ||
    normalizedPlan.value === 'demo'
)
...
const navItems = [
  ...
  },
  {
    ...
    icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z"/></svg>'
  },
  {
    id: 'history',
    label: 'Request History',
    icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3h18v18H3zM21 9H3M21 15H3M12 3v18"/></svg>'
  },
];
...
const limitCards = computed(() => [
  {
    key: 'company',
    label: 'Companies',
    count: companyCount.value,
    max: userLimits.value.companyLimit,
    colorClass: 'company-color',
    icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="2" width="16" height="20" rx="2"/></svg>'
  },
  {
    key: 'contact',
    label: 'Contacts',
    count: contactCount.value,
    max: userLimits.value.contactLimit,
  ...
  },
  {
    ...
    max: userLimits.value.reviewLimit,
...
function increment(key) {
  settingsRequestForm.value[key]++;
}

function decrement(key) {
  if (settingsRequestForm.value[key] > 0) settingsRequestForm.value[key]--;
}
...
.settings-overlay.dark-mode .plan-gate-overlay {
  background: rgba(16, 14, 20, 0.15);
}
...
.plan-gate-btn {
  ...
  box-shadow: 0 2px 10px rgba(124, 92, 78, 0.35);
...
.plan-gate-btn:hover {
  ...
  box-shadow: 0 4px 16px rgba(124, 92, 78, 0.5);
}
...
.contact-color {
  background: linear-gradient(135deg, var(--c-accent-2), #b88060);
  color: #fff;
}

.review-color {
  background: linear-gradient(135deg, #c4a030, #d4b850);
  color: #fff;
}
...
.limit-bar-fill.company-color {
  background: var(--c-accent);
}

.limit-bar-fill.contact-color {
  background: var(--c-accent-2);
}

.limit-bar-fill.review-color {
  background: #c4a030;
}
...
@media (hover: none) and (pointer: coarse) {
  ...
  .qty-btn, .filter-tab, .settings-nav-item {
    min-height: 44px;
  }
}
